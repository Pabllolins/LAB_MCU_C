...
# 8 "Padrao_ArduinoIDE.c" 2

# 1 "./Bits_PIC18F.h" 1
# 9 "Padrao_ArduinoIDE.c" 2

# 1 "./Padrao_ArduinoIDE.h" 1
# 29 "./Padrao_ArduinoIDE.h"
enum pino_etiqueta{
...
void EscreveDigital(int pino, int valor){
    ...
    if(pino <8){
        if (valor){ ((PORTA) |= (1<<(pino)));}
        else{ ((PORTA) &= ~(1<<(pino)));}
    }else if(pino<16){
        pino -=8;
        if (valor){ ((PORTB) |= (1<<(pino)));}
        else{ ((PORTB) &= ~(1<<(pino)));}
    }else if(pino<24){
        pino -=16;
        if (valor){ ((PORTC) |= (1<<(pino)));}
        else{ ((PORTC) &= ~(1<<(pino)));}
    }else if(pino<32){
        pino -=24;
        if (valor){ ((PORTD) |= (1<<(pino)));}
        else{ ((PORTD) &= ~(1<<(pino)));}
    }else if(pino<40){
        pino -=32;
        if (valor){ ((PORTE) |= (1<<(pino)));}
        else{ ((PORTE) &= ~(1<<(pino)));}
    }
...
int LeituraDigital(int pino){
    if(pino <8){
        ...
        return ((PORTB) & (1<<(pino-8)));
    }else if(pino<24){
        return ((PORTC) & (1<<(pino-16)));
    }else if(pino<32){
        return ((PORTD) & (1<<(pino-24)));
    }else if(pino<40){
        return ((PORTE) & (1<<(pino-32)));
    }
    return -1;
}
...
void ModoDoPino(int pino, int tipo){
    if(pino <8){
        if (tipo){ ((TRISA) |= (1<<(pino)));}
        else{ ((TRISA) &= ~(1<<(pino)));}
    }else if(pino<16){
        if (tipo){ ((TRISB) |= (1<<(pino-8)));}
        else{ ((TRISB) &= ~(1<<(pino-8)));}
    }else if(pino<24){
        if (tipo){ ((TRISC) |= (1<<(pino-16)));}
        else{ ((TRISC) &= ~(1<<(pino-16)));}
    }else if(pino<32){
        if (tipo){ ((TRISD) |= (1<<(pino-24)));}
        else{ ((TRISD) &= ~(1<<(pino-24)));}
    }else if(pino<40){
        if (tipo){ ((TRISE) |= (1<<(pino-32)));}
        else{ ((TRISE) &= ~(1<<(pino-32)));}
    }
